...
)  # Direct import is fine within the app
import random
import logging
...
def get_filtered_questions(
    ...
    # Ensure select_related is applied again after filtering by ID if needed, though it might persist
...
def record_user_study_activity(
    ...
    if points_update_needed:
        ...
        # Create PointLog entry (Assuming PointLog model exists in gamification app)
        # try:
    ...
    if points_update_needed or streak_update_needed:
        profile.save(
            update_fields=list(set(profile_update_fields))
        ...
        if streak_updated and profile.current_streak_days > profile.longest_streak_days:
            ...
            profile.refresh_from_db(
                fields=["longest_streak_days"]
            )  # Refresh again if needed elsewhere
...
def update_user_skill_proficiency(user, skill: Skill, is_correct: bool):
    ...
    try:
        ...
        if not created:
            proficiency.record_attempt(is_correct)  # Use the model method for updates
